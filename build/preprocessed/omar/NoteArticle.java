 ...
 * @author omarblythe
 */
class NoteArticle extends Canvas  implements  CommandListener, Runnable{
    ...
    ,Image.createImage("/img/star4.jpg"),Image.createImage("/img/star5.jpg")}  ;
            Command cmdRetour = new Command("Retour", Command.EXIT, 0);
                     ...
                     Image[] like = {Image.createImage("/img/like.png"),Image.createImage("/img/liked.png")};
...
boolean testUp = true ;
    ...
    public NoteArticle() throws IOException {
       // super("");
...
SubmitNoteArticle N = new SubmitNoteArticle(stock[selectorI].getId(),stock[selectorI].getStar()+1);
...
Login.exam.disp.setCurrent(this);
    ...
    public void run() {
        ...
        try {
            ...
            HttpConnection hc = (HttpConnection) Connector.open("http://127.0.0.1/parsing/getStock.php?user="+Login.exam.id);
            DataInputStream dis = new DataInputStream(hc.openDataInputStream());
            SAXparser.parse(dis, stockHandler);
            System.out.println(Login.exam.id);
            
            // display the result
            stock = stockHandler.getV();
  ...
  if (stock.length > 0) {
                ...
                g.drawString(stock[i].getEnseigne(), (x)-stock[i].getEnseigne().length()*8, (y/3*i+trans)+10, 0);
                g.drawString(stock[i].getNomArticle(), 5, (y/3*i+trans)+10, 0);
                g.drawString(stock[i].getGenre(), 10, (y/3*i+trans)+30, 0);
             ...
             //       System.out.println(stock[i].getStar()+"aaaaaaaaaaaaaaaaaaasssssssssssss");
...
selectorI -- ;}
        ...
        if ((gameAction == DOWN) && testDown  ) {
            ...
            else if (y/3*stock.length-1+trans <=y/3*4){
            selectorY+=y/3;
            testDown=false ;
            selectorI++;
            }
         ...
         if(gameAction==RIGHT){
               if (stock[selectorI].getStar()<4){
                System.out.println(stock[selectorI].getStar()+1);
            stock[selectorI].setStar( stock[selectorI].getStar()+1);}
            }
              if(gameAction==LEFT){
                  if (stock[selectorI].getStar()>0){
            stock[selectorI].setStar( stock[selectorI].getStar()-1);}
            } 
              if (gameAction==FIRE){
...
}else if (stock[selectorI].getLike()==1){
DeleteLikeArticle N = new DeleteLikeArticle(stock[selectorI].getId());
stock[selectorI].setLike(0);
}
else {
                    //System.out.println("idArticle = "+stock[selectorI].getId());
SubmitLikeArticle N = new SubmitLikeArticle(stock[selectorI].getId());
stock[selectorI].setLike(1);
