 ...
 * @author omarblythe
 */
class Article extends Canvas implements CommandListener, Runnable {
    ...
    int selectorY = 0;
    int selectorI = 0;
    DAO.DAOArticle dao = new DAO.DAOArticle();

    Command cmdRetour = new Command("Retour", Command.EXIT, 0);
    Command cmdNoter = new Command("ok", Command.EXIT, 0);
    ...
    public Article() throws IOException, ParserConfigurationException, SAXException {
        // super("");
       ...
       if (articles.length > 0) {
            for (int i = 0; i < articles.length; i++) {
              //  System.out.println("canvas" + i + articles[i].getNomArticle());
                ...
                g.drawString(articles[i].getNomArticle(), (x) - articles[i].getNomArticle().length() * 8, (y / 3 * i + trans) + 10, 0);
                //       g.drawString(articles[i].getNomEnseigne(), 5, (y / 3 * i + trans) + 10, 0);
                g.drawString(String.valueOf(articles[i].getPrixVente()), 10, (y / 3 * i + trans) + 30, 0);
                g.drawString(String.valueOf(articles[i].getQuantite()), 10, (y / 3 * i + trans) + 50, 0);
    ...
    protected void keyPressed(int keyCode) {
        ...
        if ((gameAction == DOWN) && testDown) {
            ...
            if (selectorY == 0) {
                selectorY = y / 3;
                selectorI++;
            } else if (y / 3 * articles.length - 1 + trans <= y / 3 * 4) {
                selectorY += y / 3;
                testDown = false;
                selectorI++;
            } else {
